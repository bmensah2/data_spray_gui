...
class DetectionPanelRGB(QWidget):
    ...
    def _draw_overlay(self, img, dual_result, spray_states):
        """
        Draw zone boundaries and detection boxes on the side-by-side
        display image. Thin wrapper around
        gui.overlay_rendering.draw_detection_overlay() -- extracted
        there so the live Detection tab and the offline video/image
        review tab draw identically from one shared implementation.
        """
        from gui.overlay_rendering import draw_detection_overlay
        return draw_detection_overlay(img, dual_result, spray_states, self._cfg)
